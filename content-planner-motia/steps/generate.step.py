...
async def handler(input: Dict[str, Any], context):
    ...
    logger.info(f"✍️ Generating social media content for: {title}")
    ...
    try:
        ...
        twitter_format = 'a Twitter thread (3-5 tweets)' if strategy['twitterStrategy']['format'] == 'thread' else 'a single engaging tweet'
        key_insights = ', '.join(strategy['analysis']['keyInsights'])
        
        twitter_prompt = (twitter_prompt_template
                         .replace('{{title}}', title)
                         .replace('{{strategy}}', json.dumps(strategy['twitterStrategy']))
                         .replace('{{keyInsights}}', key_insights)
                         .replace('{{format}}', twitter_format)
                         .replace('{{targetAudience}}', strategy['analysis']['targetAudience']))
        ...
        linkedin_prompt = (linkedin_prompt_template
                          .replace('{{title}}', title)
                          .replace('{{strategy}}', json.dumps(strategy['linkedinStrategy']))
                          .replace('{{keyInsights}}', key_insights)
                          .replace('{{targetAudience}}', strategy['analysis']['targetAudience']))
        ...
        logger.info(f"📱 Twitter: {twitter_content['totalTweets']} tweet(s)")
        logger.info(f"💼 LinkedIn: {linkedin_content['characterCount']} characters")
        ...
        await emit({
            ...
            'data': {
                ...
                'metadata': {
                    ...
                    'processingTime': int(__import__('time').time() * 1000) - timestamp,
                    'targetAudience': strategy['analysis']['targetAudience']
                }
            }
        })
